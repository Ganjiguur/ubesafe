...
//        }
//        
        // Add x-smtpapi receipients. It is to hide all sending emails from recipent
        $comma = "";
        foreach ($email->to() as $mail => $name) {
           $message['x-smtpapi'] .= $comma . ' "' . $name . ' <' .$mail.'>"';
           $comma = ",";
        }
        ...
        $this->http = new Client([
            ...
            'scheme' => 'https',
            'headers' => [
                'User-Agent' => 'Wire Mongolia CakePHP SendGrid Plugin'
            ]
        ]);
